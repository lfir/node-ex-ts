...
export default class AnalyticsController {
  ...
  storePageView = (
    ...
    if (accLangs) {
      newPageViewInfo.language = AnalyticsController.normalizeLanguage(
        accLangs
      );
    }
    if (geo) {
  ...
  static pageViewNotFoundError = (): StatusCodeError => {
    const msg =
      'No records were found in the database matching ' +
  ...
  static isOptionFromTo = (queryParams: ISearchOptions): boolean => {
    return (
      ...
      AnalyticsController.validDate(queryParams.from) &&
      AnalyticsController.validDate(queryParams.to)
    );
  };

  static isOptionLimit = (queryParams: ISearchOptions): boolean => {
    return (
      /^\d+$/.test(queryParams.limit) && Object.keys(queryParams).length === 1
    );
  };

  static isOptionFromToAndLimit = (queryParams: ISearchOptions): boolean => {
    return (
      /^\d+$/.test(queryParams.limit) &&
  ...
  retrievePageViews = async (
    ...
    switch (AnalyticsController.getChosenOptions(queryParams)) {
      ...
      case ESearchOptions.FromTo:
        retrievedPageViews = await this.searchBetweenDates(
          queryParams.from,
          queryParams.to
        );
        break;
      case ESearchOptions.Limit:
        retrievedPageViews = await this.PageView.find().limit(
  ...
  retrieveOrUpdateOrDeletePageView = async (
    ...
    if (operation === 'get') {
      ...
      resultPageView = await this.PageView.findByIdAndUpdate(
        id,
        newPageView as Object,
        { new: true } as Object
      );
  ...
  updatePageView = (
    ...
    if (newPageView.host)
      newPageView.host = newPageView.host.replace(/[^\w-.]/g, '');
    if (newPageView.path)
      newPageView.path = AnalyticsController.normalizePath(newPageView.path);
    if (newPageView.language)
      newPageView.language = AnalyticsController.normalizeLanguage(
        newPageView.language
      );
    if (newPageView.country)
      newPageView.country = newPageView.country.replace(/[^a-z]/g, '');
    if (newPageView.date && !AnalyticsController.validDate(newPageView.date))
      throw AnalyticsController.searchQueryError();
    return this.retrieveOrUpdateOrDeletePageView(
      'upd',
      queryParams,
      newPageView
    );
